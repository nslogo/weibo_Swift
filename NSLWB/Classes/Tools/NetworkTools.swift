...
extension NetworkTools {
    func loadUserInfo(access_token : String, uid : String, finished : (result : [String : AnyObject]?, error : NSError?) -> ()){
        ...
        //发送网络请求
        request(.GET, urlString: urlString, parameters: parame) { (result, error) in
            finished (result: result as? [String : AnyObject], error: error)
        }
    }
}
...
extension NetworkTools {
    func sendStatus(statusText : String, isSuccess : (isSuccess : Bool) -> ()) {
        ...
        //获取请求的参数
        let parmaetes = ["access_token" : (UserAccountViewModel.shareIntance.account?.access_token)!, "status" : statusText]
        
        //发送请求
        request(.POST, urlString: urlString, parameters: parmaetes) { (result, error) in
            if result != nil {
